...
const ACTIVE_SESSION_KEY = 'finu_active_payment_session';
const EXPIRED_STATES = new Set(['AUTH_RELEASE_PENDING', 'CANCELLED', 'EXPIRED']);
const TERMINAL_STATES = new Set(['COMPLETED', 'CARD_FAILED', 'UPI_FAILED', 'REFUND_FLAGGED', 'AUTH_RELEASE_PENDING', 'CANCELLED', 'EXPIRED', 'CARD_CAPTURE_FAILED']);
...
export default function App() {
  ...
  const [recoveredUpiOrderId, setRecoveredUpiOrderId] = useState(null);
  const [recoveredKeyId, setRecoveredKeyId] = useState(null);
  ...
  }, [cardAmount, merchantId, merchantName, orderId, orderTotal, productName, returnUrl, sessionId, upiAmount]);

  const applyBackendSession = useCallback((backendSession, saved = activeSessionRef.current) => {
    if (!backendSession) return;
    const nextState = backendSession.state || 'CREATED';
    setSessionId(backendSession.sessionId);
    setOrderTotal(saved?.orderTotal || backendSession.orderTotal || null);
    setCardAmount(saved?.cardAmount || backendSession.cardAmount || 0);
    setMerchantName(saved?.merchantName || merchantName || 'Merchant');
    ...
    if (nextState === 'COMPLETED') {
      setPhase(PHASE.DONE);
      setError('');
      saveActiveSession({ ...saved, sessionId: backendSession.sessionId, state: nextState });
      return;
    }

    if (EXPIRED_STATES.has(nextState)) {
      setPhase(PHASE.DONE);
    ...
    if (nextState === 'CARD_CAPTURE_FAILED') {
      setPhase(PHASE.DONE);
      setError(backendSession.cardCaptureError || 'UPI succeeded but card capture failed. Please contact support.');
      saveActiveSession({ ...saved, sessionId: backendSession.sessionId, state: nextState });
    }
  }, [merchantName, orderId, productName, saveActiveSession]);
  ...
  useEffect(() => {
    ...
    api.getSession(saved.sessionId)
      .then((backendSession) => applyBackendSession(backendSession, saved))
      .catch(() => clearActiveSession());
  }, [applyBackendSession, clearActiveSession]);

  useEffect(() => {
    if (!sessionId || ![PHASE.PROCESSING, PHASE.DONE].includes(phase)) return undefined;
    if (TERMINAL_STATES.has(payState)) return undefined;

    const checkSession = async () => {
      try {
        const backendSession = await api.getSession(sessionId);
        if (backendSession.state !== payState || TERMINAL_STATES.has(backendSession.state)) {
          applyBackendSession(backendSession);
        }
      } catch (err) {
        // Keep the current UI if a transient network check fails.
      }
    };
  ...
  const handleProceed = useCallback(async () => {
    ...
    try {
      if (activeSessionRef.current?.sessionId) {
        const existing = await api.getSession(activeSessionRef.current.sessionId);
        if (existing?.state === 'UPI_PENDING' || (existing && !TERMINAL_STATES.has(existing.state))) {
          applyBackendSession(existing, activeSessionRef.current);
          setLoading(false);
          return;
        }
      }

      const session = await api.createSession({ orderTotal, cardAmount, upiAmount, merchantOrderRef: orderId, customerEmail: 'demo@finu.in', customerPhone: '9000000000' });
      setSessionId(session.sessionId);
      saveActiveSession({ sessionId: session.sessionId, cardOrderId: session.cardOrderId, razorpayKeyId: session.razorpayKeyId, state: 'CARD_PENDING' });
      setPhase(PHASE.PROCESSING);
      ...
      try {
        ...
        saveActiveSession({ sessionId: session.sessionId, cardOrderId: session.cardOrderId, razorpayKeyId: session.razorpayKeyId, upiOrderId: upiSession.upiOrderId, state: 'UPI_PENDING' });
        setRecoveredUpiOrderId(upiSession.upiOrderId);
      ...
      try {
        ...
        await api.cancelSession(session.sessionId);
        saveActiveSession({ sessionId: session.sessionId, state: 'AUTH_RELEASE_PENDING' });
        setPayState('AUTH_RELEASE_PENDING');
        setError('This payment session has expired. Please start a new payment.');
        setPhase(PHASE.DONE); setLoading(false); return;
      }
      ...
      try {
        ...
        saveActiveSession({ sessionId: session.sessionId, state: 'COMPLETED' });
  ...
  const handleContinueUpi = useCallback(async () => {
    ...
    try {
      const backendSession = await api.getSession(sessionId);
      if (backendSession.state !== 'UPI_PENDING') {
        applyBackendSession(backendSession);
        setLoading(false);
        return;
      }

      const upiResponse = await openCheckout({
        key: recoveredKeyId,
        order_id: recoveredUpiOrderId,
        amount: Math.round(upiAmount * 100),
        currency: 'INR',
        name: 'FinU Technologies - Step 2 of 2',
        description: `UPI payment Rs. ${upiAmount.toLocaleString('en-IN')} - almost done!`,
        prefill: { email: 'demo@finu.in', contact: '9000000000' },
        theme: { color: '#4A90D9' },
        notes: { leg: 'UPI', sessionId, merchant_id: merchantId, order_ref: orderId },
        method: { upi: true, card: false, netbanking: false, wallet: false, emi: false },
      }, true);
      ...
      setPhase(PHASE.DONE);
      saveActiveSession({ sessionId, state: 'COMPLETED' });
    } catch (err) {
      const latest = await api.getSession(sessionId).catch(() => null);
      if (latest) applyBackendSession(latest);
      else {
        setPayState('AUTH_RELEASE_PENDING');
        setError('This payment session has expired. Please start a new payment.');
        setPhase(PHASE.DONE);
    ...
    setLoading(false);
  }, [applyBackendSession, loading, merchantId, openCheckout, orderId, recoveredKeyId, recoveredUpiOrderId, saveActiveSession, sessionId, upiAmount]);
  ...
  const handleReset = () => {
    ...
    setRecoveredUpiOrderId(null); setRecoveredKeyId(null); setSessionId(null); setPayState('CREATED'); setError(''); setLoading(false);
  };
        ...
        <div className="col-checkout">
          ...
          {(phase === PHASE.PROCESSING || phase === PHASE.DONE) && (
            <PaymentStatus
              ...
              error={error}
              onReset={handleReset}
              onContinueUpi={handleContinueUpi}
              canContinueUpi={payState === 'UPI_PENDING' && !!recoveredUpiOrderId && !!recoveredKeyId && !loading}
            />
          )}
        </div>
      </main>
